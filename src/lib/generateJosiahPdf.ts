...
import { cvHeader, cvSummary, cvJobs, cvSkills, cvEducation } from "./josiahCvData";
...
export async function generateJosiahPdf() {
  ...
  pdf.setFontSize(8.5);
  setColor(MID);
  const contactParts = [cvHeader.phone, cvHeader.email].filter(Boolean);
  let cx = MARGIN;
  ...
  pdf.save("Josiah Sithole - CV.pdf");
}
