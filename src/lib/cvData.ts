...
export const cvJobs: CvJob[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      "Built a two-phase document parsing pipeline: Phase 1 (LlamaParse `tier=agentic` → structured markdown with headings/tables/figure descriptions → LangChain MarkdownHeaderTextSplitter + RecursiveCharacterTextSplitter → Gemini embeddings + Pinecone upsert, synchronous) + Phase 2 (Tesseract/ocrmypdf background OCR for scanned pages, enables Ctrl+F). Selected LlamaParse after a systematic 5-provider evaluation (Docling, Google Document AI, Azure Document Intelligence, LlamaParse, Unstructured.io).",
